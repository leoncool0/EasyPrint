...
**客户端：**
- Windows 7 兼容支持：PySide6 降级至 6.2.x（6.5+ 移除了 Win7 支持）
- 32 位系统检测：启动时检测 CPU 架构，32 位系统弹窗提示友好错误
- 依赖版本锁定：pywin32/Pillow/PyMuPDF 限制最大版本，确保 32-bit wheel 可用
- build.bat 修复：添加 `--noupx` 参数避免 PE 校验和写入权限问题
- 架构检测在 Qt 导入前执行，使用 ctypes 弹窗，不依赖 GUI 框架
